...
function inviteAttendee(attendeeList) {
  let subject = [];
    let to = [];
    let message = [];
  requests = attendeeList.map(attendeeId=>{
    Attendee.findById(attendeeId).then(attendee=>{
      if(attendee) {
        let subject1 = "You have been invited !"
        let message1 = "Bonjour"+ attendee.prenom + ",<br><br>Vous êtes invité à un événement sur EMIEvent.!<br>Créez un compte ou connectez-vous afin de confirmer ou de refuser cette invitation.<br>Si vous avez des questions,mettez les en réponse à cet email, nous sommes là pour vous aider.<br><br>Cordialement,<br>L'Equipe EMIEvent."
...
exports.sendInvitations = (req, res, next) => {
  ...
  .then((result) => {
    inviteAttendee(req.body.attendeeList);
      console.log(event)
